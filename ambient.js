...
// Nature & lieux : vrais enregistrements embarqués dans sounds/
// (source Moodist, licences Pixabay/CC0 — voir sounds/CREDITS.md).
// Bruits scientifiques et binaural : Web Audio (par définition générés).
// =============================================================

let _audioCtx = null;
let _activeNodes = [];
let _activeAudios = [];   // éléments <audio> des enregistrements en cours
...
const MODES = {
  ...
  // Vrais enregistrements (fichiers locaux, lisibles directement dans l'app)
  rain:   { label: 'Pluie',       icon: 'rainy',        cat: 'real', files: ['sounds/light-rain.mp3'], desc: 'Pluie fine — enregistrement réel' },
  storm:  { label: 'Orage',       icon: 'thunderstorm', cat: 'real', files: ['sounds/heavy-rain.mp3', 'sounds/thunder.mp3'], desc: 'Pluie battante et tonnerre — enregistrements réels' },
  ocean:  { label: 'Vagues',      icon: 'waves',        cat: 'real', files: ['sounds/waves.mp3'], desc: 'Vagues sur la plage — enregistrement réel' },
  river:  { label: 'Rivière',     icon: 'water',        cat: 'real', files: ['sounds/river.mp3'], desc: 'Rivière qui coule — enregistrement réel' },
  forest: { label: 'Forêt',       icon: 'forest',       cat: 'real', files: ['sounds/birds.mp3', 'sounds/wind-in-trees.mp3'], desc: 'Oiseaux et vent dans les arbres — enregistrements réels' },
  wind:   { label: 'Vent',        icon: 'air',          cat: 'real', files: ['sounds/wind.mp3'], desc: 'Vent — enregistrement réel' },
...
function setAmbientVolume(v) {
  ...
  _activeAudios.forEach(a => { a.volume = _ambientVolume; });
  _saveAmbientPrefs();
  _refreshAmbientUI();
}
